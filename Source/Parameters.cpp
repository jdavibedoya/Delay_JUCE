...
static float millisecondsFromString(const juce::String& text) {
    ...
    if (!text.endsWithIgnoreCase("ms")) { 
        // 3
        if (text.endsWithIgnoreCase("s") || value < Parameters::minDelayTime) { return value * 1000.0f;
        }
    }
    return value;
}
...
static juce::String stringFromPercentage(float value, int){
...
juce::AudioProcessorValueTreeState::ParameterLayout Parameters::createParameterLayout(){
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(
            ...
            100.0f,
            juce::AudioParameterFloatAttributes() .withStringFromValueFunction(stringFromPercentage)));
    return layout;
}
